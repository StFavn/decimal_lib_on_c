...
static void convert_and_test(char *src) {
  ...
  int status_code = s21_round(in, &res);
  ld_expected = roundl(ld_expected);
...
START_TEST(zero_five_0) {
  s21_decimal actual = {0};
  s21_str_to_dec("100.5", &actual);
  s21_decimal expected = {0};
  s21_str_to_dec("100", &expected);
  s21_decimal res = {0};

  int res_code = s21_round(actual, &res);

  ck_assert_int_eq(res_code, S21_RETURN_CODE_OK);
  ck_assert(s21_is_equal(expected, res));
}

START_TEST(zero_five_1) {
  s21_decimal actual = {0};
  s21_str_to_dec("-100.5", &actual);
  s21_decimal expected = {0};
  s21_str_to_dec("-100", &expected);
...
START_TEST(test_round_ok175) {
  ...
  int res_code = s21_round(decimal, &res);

  ck_assert_int_eq(res_code, S21_RETURN_CODE_OK);
  ck_assert(s21_is_equal(decimal_check, res));
}
...
START_TEST(null_test) {
  s21_decimal val = {0};
  ck_assert_int_eq(s21_round(val, NULL), S21_RETURN_CODE_ERR);
}
END_TEST

START_TEST(bad_dec_test_0) {
  s21_decimal dec = {.bits[3] = 111};
  s21_decimal val = {0};

  ck_assert_int_eq(s21_round(dec, &val), S21_RETURN_CODE_ERR);
}
END_TEST

START_TEST(bad_dec_test_1) {
  s21_decimal dec = {0};
  s21_decimal val = {0};

  char *src = "123.5";
  s21_str_to_dec(src, &dec);
  dec.bits[3] = 0b00000000000000010000000000000111;

  ck_assert_uint_eq(s21_round(dec, &val), S21_RETURN_CODE_ERR);
}
END_TEST

Suite *suite_s21_round() {
  Suite *suite = suite_create("suite_s21_round");
  ...
  tcase_add_test(normal_cases, zero_five_0);
  tcase_add_test(normal_cases, zero_five_1);
  ...
  tcase_add_test(edge_cases, test_round_ok175);
  tcase_add_test(edge_cases, test_round_ok176);
  tcase_add_test(edge_cases, test_round_ok223);
  tcase_add_test(edge_cases, test_round_ok224);
  tcase_add_test(edge_cases, test_round_ok581);
  tcase_add_test(edge_cases, test_round_ok582);
  tcase_add_test(edge_cases, test_round_ok998);
  suite_add_tcase(suite, edge_cases);

  TCase *anomaly_cases = tcase_create("anomaly_cases");
  tcase_add_test(anomaly_cases, null_test);
  tcase_add_test(anomaly_cases, bad_dec_test_0);
  tcase_add_test(anomaly_cases, bad_dec_test_1);
