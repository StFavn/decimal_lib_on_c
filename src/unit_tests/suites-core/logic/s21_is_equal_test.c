...
START_TEST(equal_normal_test) {
  ...
  ck_assert(s21_is_equal(dec_0, dec_1));
}
END_TEST
...
START_TEST(not_equal_normal_test) {
  ...
  ck_assert(s21_is_equal(dec_0, dec_1) == false);
}
END_TEST
...
START_TEST(edge_scale_overflow_test) {
  ...
  ck_assert(s21_is_equal(dec_1, dec_2) == 0);
}
END_TEST
...
Suite *suite_s21_is_equal() {
  Suite *suite = suite_create("suite_s21_is_equal");
